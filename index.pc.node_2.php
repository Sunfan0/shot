...
<html lang="zh-CN">
	...
	<body style="overflow: hidden;padding:0px;margin:0px;">
		...
		<hr id="hrLeft" class="float" style="color:white"/>
		<hr id="hrRight" class="float" style="color:white"/>
		<div id="AdoutCode" class="float hidden"></div>
	</body>
	...
	<script type="text/javascript">
		var Settings = {};
		Settings.Balloonnumber = 500;		//气球总数
		Settings.GameOver = false;
		Settings.HitNumber = 0;
		Settings.CursorWidth = 100;
		Settings.CursorHeight = 100;
		Settings.BlowWidth = 200-200/10;
		Settings.BlowHeight = 300-300/10;
		Settings.CodeImgSrc = 'http://www.wsestar.com/test/shot/mobile_2.php';
		// Settings.CodeImgSrc = 'http://www.wsestar.com/test/shot/index.mobile.node_data.php';
		Settings.OldTop = "";
		Settings.OldLeft = "";
		Settings.CurrentTop = "";
		Settings.CurrentLeft = "";
		Settings.CurrentBalloonIndex = 0;
		...
		function TestManyJoin(){
			// setTimeout(function(){
			$("#pageBgAll").removeClass("hidden");
			$("#pageGameGo").removeClass("hidden");
			$("#pageAimContainer,#aimBalloon1,#aimBalloon2,#aimBalloon3,#aimBalloon4").removeClass("hidden");
			$("#aimCursor,#aimTitle").addClass("hidden");
			
			$("#AdoutCode").qrcode({
				render:"canvas",
				width:OffsetLeftFullScreen(212),
				height:OffsetTopFullScreen(212),
				text: Settings.CodeImgSrc
			});
			$("#AdoutCode").removeClass("hidden");
						
			BeginGame();
		// },5000)
		}
		
		function NoResponse(openid){
...
// console.log("Settings.CurrentLeft",Settings.CurrentLeft);
				if(UserWinning != [] && Settings.OldTop == Settings.CurrentTop && Settings.OldLeft == Settings.CurrentLeft && Settings.OldTop != "" && Settings.OldLeft != "" && Settings.CurrentTop != "" && Settings.CurrentLeft != ""){
					// window.location.reload();
					$("#balloonCursor"+openid).remove();
// console.log("没有响应");
				}	
			},3000);
		}
...
console.log(data.headimgurl);
				...
				beginAim();
				// setTimeout(function(){
					// setInterval(NoResponse(data.openid) , 5000);
					// if(Settings.OldTop == ""){
						// window.location.reload();
					// }
				// },60000)
				
				strImg = '<div id="balloonCursor'+data.openid+'" class="hidden float balloonCursorMany" style="z-index:9999;background-image:url('+data.headimgurl+');background-repeat: round;;background-position:left top;border-radius: 50%;opacity: 0.8;">';
				strImg+= '<img style="opacity: 0.6;width:100%;height:100%;" src="<?=$imgPath?>balloonCursor2.png"/>';
				strImg+= '</div>';
				$("#pageGameGo").append(strImg);
				
				// $(".balloonCursorMany").OffsetFullScreen({width:Settings.CursorWidth,height:Settings.CursorHeight});
				$(".balloonCursorMany").OffsetFullScreen({width:Settings.CursorWidth});
			});
...
console.log(data);
				...
				$("#balloonCursor"+data.openid).removeClass("hidden");
				
				
				UserWinning[data.openid] = 0;
			});
...
// console.log("left : " + p.l + " , top : " + p.t);
				...
				$("#balloonCursor"+data.openid).css({top : OffsetLeftFullScreen(p.t-Settings.CursorHeight/2) , left : OffsetLeftFullScreen(p.l-Settings.CursorWidth/2)});
				$("#balloonCursor"+data.openid).css("opacity","1");
...
// console.log("left : " + p.l + " , top : " + p.t);
				...
				$("#balloonCursor"+data.openid).css({top : OffsetLeftFullScreen(p.t-Settings.CursorHeight/2) , left : OffsetLeftFullScreen(p.l-Settings.CursorWidth/2)});
				$("#balloonCursor"+data.openid).css("opacity","0.1");
				
				GetBalloonPosition(data);
			});
...
console.log(UserWinning);
				...
				socket.emit('GameOver',{"result":"yes","openid":data.openid,"winNumber":UserWinning[data.openid]});
				$("#balloonCursor"+data.openid).remove(); 
				delete UserWinning[data.openid];	
console.log(UserWinning);
...
console.log(data);
		...
		function InitSize(){
			...
			$("#hrLeft").SetSizeFullScreen(-10,330,1,PageSize.oriHeight);
			$("#hrRight").SetSizeFullScreen(-10,PageSize.oriWidth-330,1,PageSize.oriHeight);
			$("#AdoutCode").SetSizeFullScreen(500,60,212,212);
		}
		...
		function BeginGame(){
			for(i=1;i<=Settings.Balloonnumber;i++){
				(function(i){
					...
					// var initialLeft = Math.floor(Math.random()*(Math.floor((PageSize.oriWidth-330)/100)-3.3+1)+3.3)*100;
					var initialLeft = Math.floor(Math.random()*(Math.floor((OffsetLeftFullScreen(PageSize.oriWidth)-OffsetLeftFullScreen(500))/100)-OffsetLeftFullScreen(4)+1)+OffsetLeftFullScreen(4))*100;
					setTimeout(function(){
						...
						$("#BalloonImg"+i).css("left",initialLeft);
