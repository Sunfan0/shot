...
<body>
	<div id="maincontainer" class="float hidden fullScreen" style="overflow:hidden;">
		...
		<div id="testPoint" style="text-align: center;" class="float readyTitleFlash">请对准屏幕左上角的气球</div>
		<div id="gameTime" class="float">
			<span>00</span>
			<span>:</span>
			<span>60</span>
			<span>:</span>
...
<script type="text/javascript">
	...
	function ClosePage(){ 
        ...
        if(a_b && window.event.clientY< 0 || window.event.altKey){
			socket.emit('GameOver',{"result":"close","openid":openid});//关闭页面
        }else{
			socket.emit('GameOver',{"result":"reload","openid":openid});//刷新页面
		}
	}
    ...
    function init(){
		...
		setTimeout(function(){
			$("#testPoint").removeClass("readyTitleFlash");
		},500)
	}
    
	
	function MobileTestPoint(p){
		// if(p>4){
			// $("#testPoint").html("开始打气球！");
			// StartGameTimer();
		// }
		$("#testPoint").addClass("readyTitleFlash");
		setTimeout(function(){
			$("#testPoint").removeClass("readyTitleFlash");
		},500)
		switch(p){
			case 1:
				$("#testPoint").html("请对准屏幕左上角的气球");
				break;
			case 2:
				$("#testPoint").html("请对准屏幕右上角的气球");
				break;
			case 3:
				$("#testPoint").html("请对准屏幕左下角的气球");
				break;
			case 4:
				$("#testPoint").html("请对准屏幕右下角的气球");
				break;
		}
	}
	function StartGameTimer(){
		...
		sec=60;
		ms=0;
		Settings.timer = setInterval(CountDown,9);
	}
	...
	function CountDown(){
		...
		if(ms==0 && sec==0 && min==0){
			...
			// socket.emit('GameOver',{"result":"over","openid":openid});
		}
		var msStr=ms;
		if(ms<10){
			msStr="0"+ms;
		...
		$('#gameTime span:eq(0)').html(minStr);
		$('#gameTime span:eq(2)').html(secStr);
		$('#gameTime span:eq(4)').html(msStr);
	}
...
// console.log(json);
			...
			// }
			// socket.emit('GetGift',{"giftname":json,"openid":openid});
			socket.emit('GetGift',{"giftname":"","openid":openid});
		// });
	}
...
// console.log(json);
	...
	function InitSocoet(){
		...
		socket.emit('SignMobile',{"openid":openid,"headimgurl":headimgurl});
...
console.log(data);
			...
			// }else{
				...
				$("#overTitle").html("恭喜你");
				$("#overNumber").html("共打了"+data.winNumber+"个");
			// }
		});
...
//console.log(event);
	...
	function InitSize(){
		...
		$("#testPoint").SetSizeFullScreen(236,74,504,84);
		$("#testPoint").css("font-size",OffsetLeftFullScreen(45)+"px");
		
		$("#gameTime").SetSizeFullScreen(450,180,504,84);
		$("#gameTime").css("font-size",OffsetLeftFullScreen(45)+"px");
	...
	function PostMobilePosition(){
		...
		socket.emit("MobilePosition",{"p":p,"openid":openid});
	}
	...
	function PostMobileShot(){
		...
		socket.emit("MobileShot",{"p":p,"openid":openid});
console.log("shot");
console.log(p);
	}
